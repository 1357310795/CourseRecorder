...
namespace CourseRecorder.Core.Aria2
{
    ...
    public class Aria2Rpc
    {
        ...
        public static async Task<bool> ConnectServer(string uri)
        {
            ...
            {
                //连接服务器
                await Aria2Rpc.webSocket.ConnectAsync(new Uri(uri), Aria2Rpc.cancellationToken);
            }
            catch
            {
                status = false;
            ...
            //检查连接是否成功
            if (Aria2Rpc.webSocket.State == WebSocketState.Open)
            {
                status = true;
            }
            return status;
        ...
        public static async Task<string> SendAndReceive(string str)
        {
            try
            {
                //发送json数据
                await Aria2Rpc.webSocket.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes(str)), WebSocketMessageType.Text, true, Aria2Rpc.cancellationToken);
                var result = new byte[1024];
                //接收数据
                await Aria2Rpc.webSocket.ReceiveAsync(new ArraySegment<byte>(result), new CancellationToken());
        ...
        public static async Task<string> AddUri(string uri)
        {
            ...
            str = await Aria2Rpc.SendAndReceive(json);
            return str;
        }
